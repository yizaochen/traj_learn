...
function run_langevin(tau, x0, n_interval, width, height, xmean, D, dt)
    println("Start simulation....")
    y_record = zeros(tau+1, 1)  # In HMM, observing variables Y(t)
    t_record = zeros(tau+1, 1)

    ypos = x0 # Initial Position, from mean position
    t = 0.
    ...
    for tau_id = 1:tau
        ...
        t_record[tau_id] = t
        for i = 1:n_interval
            F = force_doulbe_well_width_height(ypos, width, height, xmean)
...
Vref = doulbe_well_width_height(xref, width, height, xmean, d)
...
if run_simulation
    ...
    x0 = 0.
    y_record, t_record = run_langevin(tau, x0, n_interval, width, height, xmean, D, dt)
    #------------------------------------------------------------------------
